...
x_drop0_phys = 0           # initial x-position [m]
y_drop0_phys = 0           # initial y-position [m]
u_drop0_phys = 30e-3        # initial x-velocity [m/s]
...
L_phys = 10.0e-2              # half-width of square domain [m]
N = 800                      # grid points in each direction
...
frames_per_bath_period = 3  # saved frames per bath shaking period
...
gamma_sponge = sponge_strength_nd * smooth_ramp(sponge_s) if use_sponge else cp.zeros_like(X)
...
def pressure_patch_hat(F_acc, xp, yp, indentation):
    ...
    gaussian = cp.exp(-0.5 * r2 / gaussian_width**2)
