...
with an inward radial and rotational background flow. Heavy grid/FFT calculations use CuPy.

Do not save wave frames to reduce the weight of the output, as it can be 15GB or more
for bigger domains.

The term 'horizon' in this code still comes from the Schwarzschild condition, and B is
set manually for rotation. This code still needs to be adapted to use a Kerr horizon
definition, and should have a mechanism to determine B, which now is picked by the user.
So, the horizon is not yet properly defined for this case, but the droplet motion for a
given A and B should be accurate.
"""
...
filename = "PWH_AG_GPU_CircOrbit_Opt2_Azimuthal.npz"
...
# The horizon radius is chosen and A is later calculated accordingly.
r_H_phys = 5.0e-2              # chosen group horizon radius [m]

# Azimuthal circulation parameter for v_theta = B/r.
# Positive B gives counterclockwise rotation; negative B gives clockwise rotation.
B_phys = 0.0371991284                   # azimuthal flow parameter [m^2/s]
...
x_drop0_phys = 0.3429494658             # initial x-position [m]
y_drop0_phys = 0          # initial y-position [m]
u_drop0_phys = 0.02          # initial x-velocity [m/s]
v_drop0_phys = 0.30545*0.2461          # initial y-velocity [m/s]
...
L_phys = 0.40               # half-width of square domain [m]
N = 3200                        # grid points in each direction; ideal dx = 2.5e-4 m

# Outer sponge layer.
use_sponge = True
outer_sponge_width_fraction = 2.0e-2 / L_phys  # outer sponge width as a fraction of L (divide by L for m)
sponge_strength_nd = 2             # maximum sponge damping rate in nondimensional time [-]

# ------------------------------------------------------------
# Time integration / output settings
# ------------------------------------------------------------

t_end_phys = 2               # final simulation time [s]
...
B_nd = B_phys * time_scale / length_scale**2
B_over_A = B_phys / A_phys if A_phys != 0.0 else np.nan
...
# Smooth draining-vortex background flow.
# Outside the regularised drain region:
#     v_r = -A/r,    v_theta = B/r
# Positive B corresponds to counterclockwise rotation.
activation = smooth_ramp((r - r_drain) / inner_sponge_width)
r2_safe = cp.maximum(r**2, r_drain**2)

vx_B = activation * (-A_nd * X - B_nd * Y) / r2_safe
vy_B = activation * (-A_nd * Y + B_nd * X) / r2_safe
...
print("Walker horizon / background-flow setup", flush=True)
print(f"Faraday wavenumber: kF = {kF:.5e} 1/m", flush=True)
print(f"Chosen group horizon radius: r_H = {r_H_phys:.5e} m", flush=True)
print(f"Radial sink parameter: A = {A_phys:.5e} m^2/s", flush=True)
print(f"Azimuthal flow parameter: B = {B_phys:.5e} m^2/s", flush=True)
print(f"Azimuthal-to-radial ratio: B/A = {B_over_A:.5e}", flush=True)
print("", flush=True)
