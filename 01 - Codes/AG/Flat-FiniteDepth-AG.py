"""
Simulation of finite-depth gravity-wave perturbations on an inward cylindrical
background flow, with NO surface tension.
...
c_k0 = np.sqrt(g * np.tanh(k0 * H) / k0)  # finite-depth phase speed [m/s]
...
# Wavenumber arrays for the DtN operator.
...
max_signal_speed = np.max(speed_B[active]) + c_shallow
...
print(f"Finite-depth phase speed: c_k0 = {c_k0:.5e} m/s")
