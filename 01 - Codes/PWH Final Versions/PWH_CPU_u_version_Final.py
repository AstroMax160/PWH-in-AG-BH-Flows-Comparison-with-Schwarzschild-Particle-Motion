...
This version of the code uses the combined variable u, which simplifies computations
but does not allow for the inclusion of a background flow. Used to test parameters
since it runs faster than the full solver.

This version has Gaussian pressure patch and makes everything non-dimensional when solving.
"""
...
filename = "PWH_CPU_u_lowering.npz"
...
Gamma =  4.90825318                 # dimensionless peak forcing, Gamma = gamma_peak / g [-]
GammaF = 5.1665823                # Faraday threshold in the same convention [-]
...
x_drop0_phys = 0.0e-2           # initial x-position [m]
...
L_phys = 4.0e-2 #   0.11403624             # half-width of square domain [m]
N = 1024                     # grid points in each direction

# Time integration / output settings
t_end_phys = 1.5            # final simulation time [s]
...
frames_per_bath_period = 2  # saved frames per bath shaking period
save_npz = True
...
kF =  1264.8617255251  # 1322.35443365                           # Faraday wavenumber [1/m]
