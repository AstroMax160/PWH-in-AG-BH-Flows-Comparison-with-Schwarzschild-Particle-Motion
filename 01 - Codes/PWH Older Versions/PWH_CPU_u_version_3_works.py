...
filename = "PWH_CPU_u_epstar38_512.npz"
...
Gamma = 3.8                 # dimensionless peak forcing, Gamma = gamma_peak / g [-]
GammaF = 4.22                # Faraday threshold in the same convention [-]
...
# Initial droplet state, specified dimensionally for readability
x_drop0_phys = -0.02           # initial x-position [m]
y_drop0_phys = 0.0           # initial y-position [m]
u_drop0_phys = 1.0e-2        # initial x-velocity [m/s]
...
frames_per_bath_period = 10  # saved frames per bath shaking period
save_npz = True
...
kF_milewski = 12.52e2                           # Faraday wavenumber from Milewski Fig. 1 [1/m]
lambdaF_milewski = 2.0 * np.pi / kF_milewski    # Faraday wavelength diagnostic [m]
...
epsilon_nd = (nu * time_scale / length_scale**2) * 0.8025 # Epsilon star
...
def forcing_u_hat(u_hat, t, P_hat=None):
    ...
    u_t = -(i omega + 2 epsilon k^2) u
          - i (k/omega) [Gamma_acc_nd cos(2 pi t) eta_hat + Bo P_hat]
    """
    if P_hat is None:
        P_hat = 0.0
    ...
    forcing = -1j * k_over_omega * (
        Gamma_acc_nd * np.cos(2.0 * np.pi * t) * eta_hat + Bo_nd * P_hat
    )
...
print(f"Faraday wavenumber: kF = {kF_milewski:.5e} 1/m", flush=True)
...
print(f"Integrating-factor dt = {dt * time_scale:.5e} s = {dt:.5e} bath periods", flush=True)
